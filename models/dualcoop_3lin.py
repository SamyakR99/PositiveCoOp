...
class DualCoop(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.linear_layer2 = nn.Linear(128, 2*len(classnames)) 
        self.relu = nn.ReLU()
    ...
    def forward(self, image, cls_id=None):
        ...
        img_features_permuted = image_features.permute(0,2,1)
        alpha = self.linear_layer(img_features_permuted)
        alpha = self.relu(alpha)

        alpha = self.linear_layer1(alpha)
        alpha = self.relu(alpha)
        final_image_feature = self.linear_layer2(alpha)
        ...
        output = final_image_feature.permute(0,2,1)
        b, c, _ = output.shape
        
        # torch.save(output, '/home/samyakr2/Summer24/linear_layer/DualCoOp/visual_content/final_image_feature.pt')
        ...
        ##### DualCoOp Aggreagation
        # output_half = output[:,  c // 2:]
        # w_half = F.softmax(output_half, dim=-1)
        # w = torch.cat([w_half, w_half], dim=1)
        # output = 5 * (output * w).sum(-1)
        
        ##### No Attention Aggreagation
        # output = 5 * output.sum(-1)

        
        b, c = output.shape
        ##### To get back, just comment everything above till you reach the breakpoint
        # output = 5*output.sum(-1)
