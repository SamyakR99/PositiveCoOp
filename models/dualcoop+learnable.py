...
class MLCPromptLearner(nn.Module):
    ...
    def forward(self, cls_id=None):
        ...
        return prompts, tokenized_prompts, prompts_neg, prompts_pos #, tokenized_prompts_neg, tokenized_prompts_pos 
...
class DualCoop(nn.Module):
    ...
    def forward(self, image, cls_id=None):
        ...
        prompts, tokenized_prompts, prompts_neg, prompts_pos = self.prompt_learner(cls_id)
        tokenized_prompts_pos = tokenized_prompts[:len(self.classnames),:]
        tokenized_prompts_neg = tokenized_prompts[len(self.classnames):,:]
        # breakpoint()
        text_features_pos = self.text_encoder(prompts_pos, tokenized_prompts_pos)
        text_features_neg = self.text_encoder(prompts_neg, tokenized_prompts_neg)

        # total_text_features = self.text_encoder(prompts, tokenized_prompts)

        # print('text_features_pos', text_features_pos.shape)
        # print('text_features_neg', text_features_neg.shape)
        # print('total_text_features', total_text_features.shape)
